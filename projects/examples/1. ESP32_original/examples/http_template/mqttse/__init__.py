...
class MQTTClient:
    ...
    def connect(self, clean_session=True):
        ...
        print("Step 1 Start connect with: %s" % str(addr))

        if not self.sock:
            print("If Not socks: %s" % ("Try Make Sock"))

        try:
            print("Step 2 Try: %s" % ("Connect with addr"))
            self.sock.connect(addr)
        except OSError as e:
            print("Error: %s" % ("Error"))
            return None

        print("Step 3: %s" % ("Try comminicate"))
