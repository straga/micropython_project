...
class MQTTClient:
    ...
    def __init__(self, client_id, server, port=0, user=None, password=None, keepalive=0,
                 ssl=False, ssl_params={}, timeout=0, sbt=None, debug=False):
        if port == 0:
    ...
    def connect(self, clean_session=True):
        ...
        try:
            ...
            if self.debug:
                print("Connect Read Error: %s" % e)
                print("Connect Resp Error: %s" % resp)
            return None
        ...
        if self.cb:
            ...
            if op == 0x90:
                try:
                    ...
                    if self.debug:
                        print("Subscribe Read Error: %s" % e)
                    return None

                if self.debug:
                    print("Subscribe 0x90 resp: %s" % resp)
                return None
        else:
    ...
    def wait_msg(self):
        ...
        try:
            ...
            if self.debug:
                print("Wait msg error: %s" % e)
            return None

        if res is None or res == b"":
            if self.debug:
                print("Wait msg None or Empty: %s" % res)
            return None
